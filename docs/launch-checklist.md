...
## Cloudflare Workers/OpenNext 설정

Cloudflare 화면에서 배포 명령 입력이 필수인 경우 아래처럼 설정합니다.
...
- `npx wrangler deploy`를 직접 입력하지 않습니다.
- Worker 이름은 `aidailypick`으로 통일합니다.
- 저장소의 `wrangler.jsonc`가 이 이름을 고정합니다.
